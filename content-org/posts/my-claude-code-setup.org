...
This all stems from [[https://www.youtube.com/watch?v=5N-okeDdIuI][Kun Chen's Agentic Workflow video]].
...
*** tmux, tuned to my Doom reflexes
...
#+caption: tmux which-key popup showing the Doom-style leader table - +Windows, +Panes, +Sessions all one key away
[[/images/my-claude-code-setup/tmux-doom-plan.png]]
...
*** Notifications and agent visibility with herdr

This whole section used to be about a custom =notify.sh= hook I wired up with osascript - pretty proud of it at the time.
But I've consolidated the whole problem by switching to [[https://herdr.io][herdr]], a multiplexer that is agent-aware from the ground up.
...
#+caption: herdr showing three agents across two spaces - swish General, swish Blog, and personal - all labeled and visible at a glance
[[/images/my-claude-code-setup/herdr.png]]
...
#+caption: herdr notification - clickable, takes you directly to the agent that posted it
[[/images/my-claude-code-setup/herdr-notification.png]]

*** AXIs - Agent Experience Interfaces
...
#+caption: lavish in action - the tmux config plan with my feedback visible in the conversation panel
[[/images/my-claude-code-setup/lavish-plan.png]]

*gh-axi* this just wraps the github cli, apparently its more efficient, but we shall see lol.

*** Skills I borrowed from Matt Pocock
...
#+caption: The /teach skill running a lesson on Claude agent primitives
[[/images/my-claude-code-setup/tmux-teach.png]]

*** Voice with OpenSuperWhisper
